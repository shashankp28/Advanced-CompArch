...
current=$(pwd)
cp ./inconsequence.cpp $home/$version/source/tools/ManualExamples/inconsequence.cpp
cd $home/$version/source/tools/ManualExamples
make inconsequence.test

tool="$home/$version/source/tools/ManualExamples/obj-intel64/inconsequence.so"
...
for cmd in "${commands[@]}"; do
    ...
    mv inconsequence.out $current/data/$name.count
    ((count+=1))
done
